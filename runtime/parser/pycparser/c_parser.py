...
class CParser(PLYParser):
    ...
    def _build_kernel_definition(self, spec, decl, config, param_decls, body):
        """ Builds a kernel definition.
        """
        assert 'typedef' not in spec['storage']
    ...
    def p_kernel_definition(self, p):
        """ kernel_definition   : __VE_KERNEL__ LPAREN argument_expression_list RPAREN declaration_specifiers id_declarator declaration_list_opt compound_statement
                                | __VE_KERNEL__ declaration_specifiers id_declarator declaration_list_opt compound_statement
        """

        if(len(p) == 8):
            # first case, with configuration

            spec = p[5]

            p[0] = self._build_kernel_definition(
                spec=spec,
                decl=p[6],
                config=p[3],
                param_decls=p[7],
